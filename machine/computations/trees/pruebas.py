...
reels = ["SASA", "ASBSA", "BASBBBBSBA", "ASAA", "AASBA"]
...
for key in FSReelsDict:
    ...
    key_file = open(key + "pruebas_" +".json", "a")
    key_file.write(str(key_total))
    key_file.close()
    if key_total[2]:
        print(key, key_total)
tot_file = open("pruebas_total.json", "a")
tot_file.write(str(gm_Total))
tot_file.close()
all_keys_file = open("pruebas_allKeysGMP.json", "a")
all_keys_file.write(str(FSGMPDict))
all_keys_file.close()
print("pruebas_gmtotal", gm_Total)
# print("end")